...
async def test_new_access_token(
    ...
    me_response_success: Response = await fastapi_client.get(
        "/me", headers={"Authorization": f"Bearer {get_new_access_token}"}
    )
    ...
    assert UUID(me_response_success_json["id"]) == create_user_modulescoped[3]
...
async def test_old_access_token(
    ...
    me_response_fail: Response = await fastapi_client.get(
        "/me", headers={"Authorization": f"Bearer {create_user_modulescoped[1]}"}
    )
...
async def test_new_access_token_with_old_refreshtoken(
    ...
    refresh_request_fail_json: dict = {"refresh_token": create_user_modulescoped[1]}
